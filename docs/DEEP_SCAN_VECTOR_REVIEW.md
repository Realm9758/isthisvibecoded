...
| --- | --- | --- | --- |
| Client-code secrets | Reads the submitted HTML, preload/modulepreload scripts, and bounded concrete same-origin chunk references under a transport-enforced 2 MB aggregate cap; distinguishes material secrets from public/provider keys and placeholders. | Strong evidence for validated secrets | Add framework-specific asset-manifest parsing and correlate every provider credential to the discovered project. |
...
| Security headers | Evaluates effective CSP, HSTS, framing, MIME, referrer, and permissions-policy values and samples up to three discovered routes for policy drift. | Configuration review | Add authenticated route samples and compare duplicate-header handling at the raw transport layer. |
...
| CORS | Sends hostile and null origins to the page and selected API routes, exercises OPTIONS preflights, and checks reflected-origin cache variation; high severity requires reflected credentials plus sensitive-looking JSON. | Useful indicator | Repeat with an authenticated test session and owner-selected method/header variants. |
...
| Admin access | Prioritises management routes found in the application, then probes a bounded conventional inventory and requires privileged-interface content with no visible login gate. | Useful indicator | Confirm functionality with a non-mutating browser action before high severity. |
...
| Directory listing | Checks directories learned from discovered routes plus selected common directories and requires real index-listing markup. | Strong evidence for tested paths | Add source-map and robots-derived directory candidates with catch-all baselines. |
...
| Diagnostic consoles | Requires product-specific content at bounded Apache, nginx, PHP-FPM, and Spring Actuator paths. | Strong evidence for confirmed product output | Add framework-fingerprinted Rails and cloud-health candidates without expanding blind path volume. |
| API documentation | Prioritises documentation routes found in browser code and requires structured OpenAPI/Swagger/ReDoc evidence at bounded paths. | Useful indicator | Use confirmed schemas as a route/parameter inventory for later selected modules, with method and mutation safety classification. |
| GraphQL introspection | Prioritises discovered GraphQL paths, then sends a bounded schema query to conventional endpoints and requires structured schema data. | Useful indicator | Parse the schema to find read-only operations, test field-level authorization with owner accounts, and avoid treating intentional public introspection as a flaw. |
