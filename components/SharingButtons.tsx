...
export function SharingButtons({ scanId, vibeScore, vibeLabel, securityScore, domain }: Props) {
  ...
  const tweetText = encodeURIComponent(
    ...
    `Public provenance evidence: ${vibeScore}/100 (${vibeLabel})\n` +
    `Header hardening index: ${securityScore}/100\n\n` +
    `Check your own site →`
  );
  ...
  const redditUrl = `https://reddit.com/submit?url=${encodeURIComponent(resultUrl)}&title=${encodeURIComponent(`${domain} provenance evidence index: ${vibeScore}/100 — VibeScan`)}`;
